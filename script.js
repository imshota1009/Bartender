...
document.addEventListener('DOMContentLoaded', () => {
    try {
        ...
        function setLanguage(lang) {
            ...
            generateIngredients(); // Was populateShelf
            renderUpgrades();
            ...
            // === Update Phase 2 displays ===
            if (typeof updateWeatherDisplay === 'function') updateWeatherDisplay();
            if (typeof checkSeasonalEvent === 'function') checkSeasonalEvent();
            if (typeof updateTitleDisplay === 'function') updateTitleDisplay();

            // === Update current customer name/dialogue for new language ===
            if (state.isGameRunning && state.currentCustomer) {
                const newLangData = translations[lang];
                // Find matching customer in new language by image path
                const allCustomers = [
                    ...newLangData.CUSTOMERS,
                    newLangData.MILLIONAIRE_CUSTOMER,
                    newLangData.DEATH_CUSTOMER
                ];
                const matchingCustomer = allCustomers.find(c => c && c.image === state.currentCustomer.image);
                if (matchingCustomer) {
                    // Update customer name and quotes to new language
                    state.currentCustomer.name = matchingCustomer.name;
                    state.currentCustomer.quotes = matchingCustomer.quotes;
                    // Update displayed name
                    const customerNameEl = document.getElementById('customer-name');
                    if (customerNameEl) customerNameEl.textContent = matchingCustomer.name;
                    // Hide current dialogue (displayed text can't be translated live)
                    // It will show new language text on next interaction
